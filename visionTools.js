...
async function runInspectionCycle(isManualTrigger) {
  ...
  lastToolVisualData = {};
  const inlineMsgReset = document.getElementById('runErrorInlineMsg');
  if (inlineMsgReset) inlineMsgReset.classList.add('d-none');
  ...
  try {
    ...
    if (res.success === false) {
      ...
      _showRunErrorDetail(res.error || 'Server menolak permintaan inspeksi tanpa keterangan detail.');
    ...
    badgeEl.innerText = 'ERROR';
    badgeEl.className = 'run-decision-badge ng';
    _showRunErrorDetail(err.message || String(err));
    if (btnRecapture) btnRecapture.disabled = false;
  }
}

/**
 * Tampilkan pesan error inspeksi yang SEBENARNYA ke layar (panel yang sama
 * dengan keterangan NG), bukan cuma badge "ERROR" polos + console.warn yang
 * tidak akan pernah dilihat operator di lantai produksi (mereka tidak buka
 * DevTools). Sebelumnya operator cuma lihat "ERROR" tanpa tahu sebabnya -
 * jadi tidak bisa membedakan gagal kamera, gagal jaringan, recipe rusak,
 * dll. DITAMPILKAN DI 2 TEMPAT SEKALIGUS supaya tetap kelihatan walaupun
 * panel sidepanel kanan kebetulan terpotong/tidak kelihatan di layar HP
 * tertentu (mis. karena style.css versi lama masih ke-cache): panel
 * runNgDetail di sidepanel, DAN sebagai pita merah langsung di atas video.
 */
function _showRunErrorDetail(message) {
  const text = '⚠️ Inspeksi GAGAL: ' + String(message);
  const detailDiv = document.getElementById('runNgDetail');
  if (detailDiv) detailDiv.innerHTML = '<span class="text-danger">' + text + '</span>';

  const inlineMsg = document.getElementById('runErrorInlineMsg');
  if (inlineMsg) {
    inlineMsg.innerText = text;
